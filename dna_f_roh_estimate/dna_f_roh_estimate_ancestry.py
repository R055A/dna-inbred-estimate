...
class InbredEstimateAncestry(InbredEstimateAbstractTxt):
    ...
    def _get_alleles_data(self, header: dict[str, int], data_row: list[str]) -> list[str | None]:
        ...
        """
        alleles_one: str = data_row[header[DnaKitFileHeaderAncestry.HEADER_ALLELE_ONE.value]].strip().upper()
        alleles_two: str = data_row[header[DnaKitFileHeaderAncestry.HEADER_ALLELE_TWO.value]].strip().upper()
